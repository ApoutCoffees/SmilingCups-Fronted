  ...
  <div class="checkout-container">
    ...
    <div class="checkout-content">
      <div class="cart-details">
        ...
        <div v-if="cart.selectedPlan.value" class="cart-item">
          <div class="item-info">
            <h3>{{ cart.selectedPlan.value.name }} Plan</h3>
            <p>{{ cart.selectedPlan.value.billing }}</p>
          </div>
          <span class="item-price">${{ cart.selectedPlan.value.price.toFixed(2) }}</span>
        </div>
        <div v-else class="empty-cart">
          <p>{{ t('checkout.empty_cart') }}</p>
          <router-link to="/subscriptions" class="btn btn-secondary">{{ t('checkout.go_to_subscriptions') }}</router-link> </div>
      </div>
