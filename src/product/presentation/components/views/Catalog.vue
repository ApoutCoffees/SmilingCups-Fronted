...
const { allCafes, loading, error, originsOptions, flavorNotesOptions, fetchCoffees } = useProductStore();
...
const filteredCafes = computed(() => {
  ...
  if (searchTerm.value) {
    ...
    filtered = filtered.filter(cafe =>
        ...
        (cafe.place?.toLowerCase().includes(searchLower))
    );
  }

  if (selectedPlans.value.length > 0) {
    filtered = filtered.filter(cafe => selectedPlans.value.includes(cafe.minSubscription));
  }

  if (selectedOrigins.value.length > 0) {
    filtered = filtered.filter(cafe => selectedOrigins.value.includes(cafe.originKey));
  }

  if (selectedRoasts.value.length > 0) {
    // CAMBIO: Usamos 'toasted' para coincidir con la Entidad actualizada
    filtered = filtered.filter(cafe => selectedRoasts.value.includes(cafe.toasted));
  }

  if (selectedFlavorNotes.value.length > 0) {
    filtered = filtered.filter(cafe =>
        Array.isArray(cafe.notes) && selectedFlavorNotes.value.some(note => cafe.notes.includes(note))
    );
  }

  filtered.sort((a, b) => {
    if (sortBy.value === 'name-asc') { return (a.name || '').localeCompare(b.name || ''); }
    else if (sortBy.value === 'name-desc') { return (b.name || '').localeCompare(a.name || ''); }
    else if (sortBy.value === 'price-asc') { return (a.price || 0) - (b.price || 0); }
    else if (sortBy.value === 'price-desc') { return (b.price || 0) - (a.price || 0); }
    return 0;
  });
    ...
    <main class="catalog-content">
      ...
      <div v-else class="cards-container">
        <div v-for="cafe in filteredCafes" :key="cafe.id" class="cafe-card">
          <img :src="cafe.imageUrl" :alt="'Taza de café ' + cafe.name" class="card-image">
          <div class="card-content">
            <h3>{{ cafe.name }}</h3>
            <p class="origin">{{ cafe.place }}</p>
            <p class="description">{{ cafe.description }}</p>
            <div class="notes">
              <strong>{{ t('catalog.notes_label') }}</strong>
              <span v-for="nota in cafe.notes" :key="nota" class="note-tag">{{ nota }}</span>
            </div>
            ...
            <div class="view-plans-button">
              <button @click="subscribeWithPlan(cafe.minSubscription)" class="btn btn-secondary">
